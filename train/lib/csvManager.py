...
def createCSVWithConditions(sourceFolder, destinationFolder=None, cond=dict(), verbose=True):
    ...
    if verbose:
        print "Generating csv with conditions\n"
    ...
    origColumns = ['Codigo', 'Fecha (AAAA-MM-DD)', 'Hora (HHMM)',
    ...
    normpath = os.path.join(reverseFolder, str(
        conditions['dateStart']) + str(conditions['dateEnd']) + '.json')
...
if __name__ == "__main__":
    ...
    createCSVWithConditions(
        '../data/csvFiles', '../data/csvWithCondition', conditions)
